...
children.push(body('Prepared 25 September 2026. Architecture only; build plan and field mapping follow once the VAMP payload is available. The table says which out-of-box code the design uses and which it cannot; the figure on the next page shows the whole path, in the shape of the CDP IVR inbound build with the application-vulnerability API in place of the infrastructure one.', { size: 18 }));
...
children.push(table(['Component', 'Out of the box', 'In this story'], [
  ['Stream Connect on Hermes', 'Hermes, ServiceNow’s own Kafka cluster (hermes-internal); Kafka topic, subscription, topic alias, stream, script consumer (sys_kafka_* and sys_sc_topic_alias)', 'Configured. VAMP publishes to the Hermes topic sn_usem_vamp_inbound (exists on dev); alias, stream and consumer live in the application scope. No external cluster.'],
  ...
  ['Integration framework records', 'sn_sec_int_integration, sn_sec_int_impl, sn_vul_int_fw_integration_run and _process (or classic sn_vul_integration_run / _process; the API accepts both)', 'Integration and instance created once; run and process written per message by onStart, counts by onComplete, so the import is auditable like a scheduled one.'],
