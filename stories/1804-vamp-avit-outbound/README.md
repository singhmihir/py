...
five properties, the CDP architecture (topic property, one field property per table, envelope and
rendering in code), the payload fields exactly the rows of the sheet "SN to VAMP" (`SN to VAMP
Mapping.xlsx`, kept here). Built and tested in the stand-in scope on the PDI, delivered as
...
- `BOFASIVampOutboundProcessor.js` — `buildPayload(record)` returns the JSON text
  ...
  item's `assessment_request`. Each section's fields come from `usem.vamp.fields.<table>` in the
  CDP line format and parser (`servicenow_field=json_field,`), values rendered as the sheet types
  them: references as the sys_id, integers and strings as stored, date/times `MM-dd-yyyy HH:mm:ss`;
...
- `extract_mapping.py` reads the workbook and generates `vamp_mapping.json` (the 13 rows),
  `properties.json` (topic plus the four field properties, names exactly as the sheet) and
...
Drivers: `build_1804.py` (set `SNOWUSEMTP-1804_MS_VAMP AVIT Outbound Payload_V1.2`, fresh set, the
properties of V1.0/V1.1 removed under the scope's Default set, records captured explicitly, scope
audit), `fixtures_1804.py` (two fixture items, rules off: one linked to every source, one bare),
`test_1804.py` (47 checks per run, run twice: the five properties equal the sheet rows, the payload
...
(`VAMP AVIT Outbound Payload - Records.xml`: the eight records re-pointed to the client application,
...
V1.0 (sections property plus one field property per section) and V1.1 (one property with dotted
paths, related sections added beyond the sheet) were replaced the same day: the payload must hold
exactly the sheet fields, references as sys_ids, the AIT from the task's `primary_ait`.
