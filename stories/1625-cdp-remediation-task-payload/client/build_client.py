...
else { us.initialize(); us.setValue('name', %s); us.setValue('application', %s); us.setValue('description', 'Client copy of the remediation task payload builder reading JSON field mappings, built here in the stand-in scope.'); us.insert(); }
...
si.setValue('description', 'Builds the outbound Kafka payload for one remediation task; fields per table from the JSON property usem.cdp.remtask.fields.<table>.');
...
gs.print('X::' + JSON.stringify(__d));''' % (json.dumps(bool(ST.get('set'))), json.dumps(ST.get('set', '')), json.dumps(NAME), json.dumps(SCOPE), json.dumps(SCOPE), json.dumps(script)), scope=SCOPE)
