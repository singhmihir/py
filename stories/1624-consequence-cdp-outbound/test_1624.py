...
pdi = lambda s: s.replace(CLIENT_TABLE_PREFIX, PDI_TABLE_PREFIX).replace('x_boar_bofa_usem_1', P)
...
o.expect = {sys_id: a.getUniqueValue(), number: raw(a, 'number'), state: raw(a, 'state'), state_display: '' + a.getDisplayValue('state'), level: raw(a, 'u_consequence_level'), party: raw(a, 'u_accountable_party'), comments: raw(a, 'u_comments'),
    ...
    ci: '' + a.getDisplayValue('cmdb_ci'), ci_id: raw(a, 'cmdb_ci'), ait: '' + a.getDisplayValue('u_bofa_ait'), ait_id: raw(a, 'u_bofa_ait'), rejection: raw(a, 'u_rejection_reason'), mod: parseInt(a.getValue('sys_mod_count'))};
...
for run in (1, 2):
    ...
    check('A references rule, cmdb_ci and bofa_ait as display values, not sys_ids', f['rule'] == x['rule'] == 'CQR-FIXTURE-001' and f['cmdb_ci'] == x['ci'] == 'Trade Processing Portal' and f['bofa_ait'] == x['ait'] == 'AIT57152' and len(x['rule_id']) == len(x['ci_id']) == len(x['ait_id']) == 32, (f, x))
...
o.expect = {ci: '' + a.getValue('cmdb_ci'), ci_display: '' + a.getDisplayValue('cmdb_ci'), rule: '' + a.getValue('u_rule'), rule_display: '' + a.getDisplayValue('u_rule'), state: '' + a.getValue('state'), state_display: '' + a.getDisplayValue('state'), enf: '' + a.getValue('u_enforcement_status'), enf_display: '' + a.getDisplayValue('u_enforcement_status'),
...
gs.print('X::' + JSON.stringify(o));''' % dict(cons=json.dumps(CONSEQUENCE), id=json.dumps(FX['linked'])))
    ...
    check('D reference fields render the display value, not the sys_id', d['cmdb_ci'] == x['ci_display'] == 'Trade Processing Portal' and d['cmdb_ci'] != x['ci'] and d['u_rule'] == x['rule_display'] == 'CQR-FIXTURE-001' and len(x['rule']) == 32, (d, x))
