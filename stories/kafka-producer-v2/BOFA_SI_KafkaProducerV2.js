...
BOFA_SI_KafkaProducerV2.prototype = {
    ...
    _topicSysId: function(table) {
        ...
        var property = this.TOPIC_PROPERTIES[table];
     ...
     * equals envelope.element_count, a whole number (or its digits as text); throws otherwise.
    ...
    _checkElements: function(message) {
        ...
        var count = message[this.ENVELOPE_KEY].element_count;
        if (!this._isWholeNumber(count))
            throw new Error(this.ENVELOPE_KEY + '.element_count is not a whole number: ' + JSON.stringify(count));
        if (Number(count) !== elements.length)
    ...
    /**
     * Tells whether a value is a whole number of zero or more, given as a number or as digits.
     * @param {*} value - the value to check
     * @returns {Boolean} true for 0, 1, 2 ... or "0", "1", "2" ...
     */
    _isWholeNumber: function(value) {
