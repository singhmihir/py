...
NAME = 'SNOWUSEMTP-895_MS_Qualys CI Lookup Rules_V3.2'
DESC = ('Three CI lookup rules added to the USEM Qualys chain for hosts the existing chain cannot resolve: 420 Management '
        ...
        'lists each rule relies on are declared in the script itself; no system property is read or shipped. Each rule '
        'accepts a CI only when exactly one candidate remains. Applies on top of V2.3.')
...
if (us.next()) { o.set = us.getUniqueValue(); if (us.getValue('state') != 'in progress') { us.setValue('state', 'in progress'); us.update(); } }
...
gs.print('X::' + JSON.stringify(o));''' % (json.dumps(NAME), json.dumps(NAME), json.dumps(DESC), json.dumps(rules), json.dumps(QUALYS), json.dumps(QUALYS), json.dumps(QUALYS)))
print('set:', d['set']); print('\n'.join(d['rules'])); print('captured:'); print('\n'.join(d['rows'])); print('active chain:', ' > '.join(d['chain']))
assert len(d['rows']) == 3 and all('| Global |' in r and 'sn_sec_cmn_ci_lookup_rule' in r for r in d['rows']) and all(r.endswith('stored') for r in d['rules'])
json.dump({'set': d['set'], 'name': NAME, 'rows': 3, 'file': 'Qualys CI Lookup Rules V3 - Update Set.xml'}, open(os.path.join(HERE, 'state_v3.json'), 'w'), indent=1)
print('DEPLOYED: 3 rules captured, all Global, no properties')
