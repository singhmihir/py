...
NAME = 'SNOWUSEMTP-895_MS_Qualys CI Lookup Rules_V3.0'
DESC = ('Three CI lookup rules added to the USEM Qualys chain for hosts the existing chain cannot resolve: 420 Management '
        ...
        'system property. Each rule accepts a CI only when exactly one candidate remains. Applies on top of V2.2.')
...
gs.print('X::' + JSON.stringify(o));''' % (json.dumps(NAME), json.dumps(NAME), json.dumps(DESC), json.dumps(props), json.dumps(rules), json.dumps(QUALYS), json.dumps(QUALYS), json.dumps(QUALYS)))
print('set:', d['set']); print('\n'.join(d['props'])); print('\n'.join(d['rules'])); print('captured:'); print('\n'.join(d['rows'])); print('active chain:', ' > '.join(d['chain']))
json.dump({'set': d['set'], 'name': NAME}, open(os.path.join(HERE, 'state_v3.json'), 'w'), indent=1)
