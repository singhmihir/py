...
CASES = [  # label, payload, expected chain rule ('' = no match), expected CI, (optional) rule alone and its expected CI
    ...
    ('three members behind the virtual server -> declined, the service rule attaches the virtual server', P('10.230.1.20', F5, 'vip-many.bankofamerica.com'), '460', 'vip-many@10.230.1.20+pool', '455', ''),
    ...
    ('member address carried by two differently named device records -> declined, the virtual server record', P('10.230.1.60', F5, 'vip-dup.bankofamerica.com'), '460', 'vip-dup@10.230.1.60+pool', '455', ''),
    ('the only member is the load balancer itself -> declined, the virtual server record', P('10.230.1.70', F5, 'vip-lbonly.bankofamerica.com'), '460', 'vip-lbonly@10.230.1.70+pool', '455', ''),
