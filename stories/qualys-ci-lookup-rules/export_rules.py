...
ST = json.load(open(os.path.join(HERE, 'state.json'))); SET = ST['set']; NAME = ST['name']
OUT = os.path.join(HERE, 'Qualys CI Lookup Rules - Update Set.xml')
ui = SNUI(); ui.app('global')
d = ui.js('''
var o = {rows: []};
var ux = new GlideRecord('sys_update_xml'); ux.addQuery('update_set', %s); ux.orderBy('target_name'); ux.query();
while (ux.next()) o.rows.push('' + ux.getValue('target_name') + ' | ' + ux.application.getDisplayValue() + ' | ' + ((('' + ux.getValue('payload')).indexOf('WHY THIS RULE SITS AT ORDER') > -1) ? 'rewritten' : 'OLD TEXT'));
...
assert d['name'] == NAME and d['app'] == 'Global' and len(d['rows']) == 16 and all(r.endswith('| Global | rewritten') for r in d['rows'])
...
assert not hits and len(root.findall('sys_update_xml')) == 16
...
assert len(d3['sets']) == 1 and len(d3['sets'][0]['names']) == 16 and d3['sets'][0]['app'] == 'Global' and d3['local_rows_intact'] == 16
...
idx.append({'name': NAME, 'app': 'global', 'rows': 16, 'exported': 16, 'file': fname, 'created': root.find('sys_remote_update_set/sys_created_on').text if root.find('sys_remote_update_set/sys_created_on') is not None else '', 'scrub': []})
