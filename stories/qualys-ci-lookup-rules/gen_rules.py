...
Each script opens with a short header (what the rule matches, what it reads, what it
returns, where it sits in the chain) and carries a note on the stages that do the
matching. Plumbing lines get a single short comment or none.
"""
import os, textwrap
...
def stage(title, text, indent='    '):
    """A stage note: a short ruled title line and a few lines of explanation."""
    head = indent + '// -- ' + title + ' '
...
def header(name, purpose, source, reads, result, before, reaches, after):
    w = W - 3
    L = ['/* ' + name, '   ' + '-' * w]
    L += ['   ' + l for l in wrap(purpose, w)]
    L.append('')
    L += ['   ' + l for l in wrap('sourceValue is the %s field%s.' % (source, ('; the rule also reads %s from sourcePayload' % reads) if reads else ''), w, first='Input  : ', rest='         ')]
...
def serial_prep(value):
    return '\n'.join([
        "    var serial = ('' + sourceValue).trim();       // e.g. \"%s\"" % value,
        c('Placeholder serials that vendors ship on thousands of machines ("To be filled by O.E.M.", "0123456789") would match dozens of CIs, so they are refused, as is anything shorter than four characters.'),
...
def fqdn_prep(value, ip=None):
    L = ["    var fqdn = ('' + sourceValue).trim().toLowerCase();   // e.g. \"%s\"" % value,
         "    if (fqdn.indexOf('.') == -1)                  // a bare label is left to the hostname rules",
         '        return null;']
    if ip:
        L.append("    var ip = sourcePayload.IP ? '' + sourcePayload.IP : '';   // only used to break a tie")
    return '\n'.join(L)


def split_prep(value, host, domain):
    return '\n'.join([
        "    var full = ('' + sourceValue).trim().toLowerCase();   // e.g. \"%s\"" % value,
...
def ip_prep(ip):
    return '\n'.join([
        "    var ip = ('' + sourceValue).trim();           // e.g. \"%s\"" % ip,
...
def class_stage(os_text, cls, label, fallback):
    return '\n'.join([
        stage('Class from the scanned OS',
              'The search below stays inside the class the OS points at (sub-classes included), so a Red Hat host can only land on a Linux Server and a Windows Server carrying the same value is never seen. "%s" gives %s (%s). An unknown or multi-guess OS gives no class and the rule declines; %s takes over.' % (os_text, cls, label, fallback)),
        CLASSFOR,
...
def class_pref(os_text, cls, label):
    return '\n'.join([
        stage('Class the scanned OS implies, kept as a preference',
              'This rule searches the whole hardware tree, so the class is not a filter here; it is checked at the end to reject a CI whose class contradicts the scan. "%s" gives %s (%s); an unknown OS gives no class and then no check is made.' % (os_text, cls, label)),
        CLASSFOR,
        '    var pref = classFor(sourcePayload.OS);',
    ])
...
def pick_fqdn(scope_text, run_expr, run_desc):
    return '\n'.join([
...
def pick_combo(scope_text, run_expr, run_desc):
    return '\n'.join([
...
def owners_stage(var, owner_expr, what):
    return '\n'.join([
...
def rule_180():
    h = header('USEM Serial Number Hardware Match',
        ...
        'USEM Serial Number Class Match already tried the serial inside the class the OS implies.',
        'hosts with a serial whose OS is unknown (an unauthenticated scan listing several guesses, such as "%s") or whose CI is classed differently.' % MULTI,
        'the phone rule and the name rules. A serial that is not unique in the whole hardware tree is never used.')
    body = [OPEN, CHECK, serial_prep('MXQ13005TC'), IGNORE_BLOCK,
...
def rule_200():
    ...
    body = [OPEN, CHECK,
        stage('Recognise the phone label and rebuild the MAC address',
              'The first label of the DNS name must be exactly "sep" plus twelve hex characters; a server called "sepulveda01" must not be treated as a phone. The MAC is rebuilt in the four spellings the CMDB holds, because discovery tools and the call manager export write it differently ("64:F6:9D:D5:C9:B0", "64:f6:9d:d5:c9:b0", "64F69DD5C9B0", "64f69dd5c9b0").'),
        ...
        stage('Find the one phone that carries the MAC',
              'Three places are tried in turn: a network adapter with that MAC whose owner is an IP Phone CI (discovery usually stores the MAC on the adapter), the mac_address field on the phone record itself, and finally a phone named with the Unified CM device name ("SEP64F69DD5C9B0"). Each place must yield exactly one phone; two phones on one MAC or one device name is never guessed. Checking that the adapter owner really is an IP Phone is what keeps a switch or server adapter with a colliding MAC out.'),
...
def rule_250():
    ...
    body = [OPEN, CHECK, fqdn_prep(value, True), IGNORE_BLOCK,
        class_stage('VMware ESXi 7.0.3 build 24723872', 'cmdb_ci_esx_server', 'ESX Server', 'the hardware-wide FQDN match'),
        pick_fqdn('of that class', 'pref', 'exact FQDN inside the class chosen'),
        CLOSE]
...
def rule_260():
    ...
    h = header('USEM FQDN Hardware Match',
        ...
        'USEM FQDN Class Match tried the exact FQDN inside the class the OS implies.',
        'hosts whose OS gives no class (unauthenticated scans with a multi-guess OS) or whose CI is classed differently from the OS.',
        'the hostname-plus-domain rules, for CIs that carry no fqdn value at all.')
    body = [OPEN, CHECK, fqdn_prep(value, True), IGNORE_BLOCK,
        pick_fqdn('under Hardware', "'cmdb_ci_hardware'", 'exact FQDN anywhere under Hardware'),
        CLOSE]
...
def rule_300():
    ...
    body = [OPEN, CHECK, split_prep(value, 'ah-1047132-001', 'sdi.corp.bankofamerica.com'), IGNORE_BLOCK,
        class_stage('Red Hat Enterprise Linux 9.8', 'cmdb_ci_linux_server', 'Linux Server', 'the hardware-wide hostname-plus-domain match'),
        pick_combo('of that class', 'pref', 'hostname plus domain inside the class chosen'),
        CLOSE]
...
def rule_310():
    ...
    h = header('USEM Hostname Domain Hardware Match',
        ...
        'USEM Hostname Domain Class Match tried the same evidence inside the class the OS implies.',
    ...
    body = [OPEN, CHECK, split_prep(value, 'lrche01xtrapd01', 'sdi.corp.bankofamerica.com'), IGNORE_BLOCK,
        pick_combo('under Hardware', "'cmdb_ci_hardware'", 'hostname plus domain anywhere under Hardware'),
        CLOSE]
...
def rule_410():
    ...
    h = header('USEM Hostname Hardware Match',
        ...
        'USEM Hostname Class Match required the short name to be unique inside the class the OS implies.',
...
def rule_700():
    h = header('USEM IP Class Match',
        ...
        'all serial and name rules. A host without a DNS name makes every name rule decline.',
...
def rule_705():
    h = header('USEM IP Hardware Match',
        ...
        'USEM IP Class Match required the address to belong to one CI of the class the OS implies.',
        'DNS-less hosts whose CI is classed generically or whose OS gave no class.',
        'USEM IP Adapter Match and USEM IP Layered Match.')
    body = [OPEN, CHECK, ip_prep('30.162.178.21'), IGNORE_BLOCK,
        class_pref('VMware ESXi 7.0.3 build 24723872', 'cmdb_ci_esx_server', 'ESX Server'),
        GENERIC, LB_HELPER,
...
def rule_730():
    h = header('USEM IP Adapter Match',
        ...
        'the address rules so far looked at the ip_address field of the CI record itself.',
        'hosts whose address is stored on an adapter record only.',
        'USEM IP Layered Match and then the last-resort name rule.')
    body = [OPEN, CHECK, ip_prep('30.162.178.21'), IGNORE_BLOCK, LB_HELPER,
...
def rule_740():
    h = header('USEM IP Layered Match',
        ...
        'USEM IP Adapter Match looked for the address on the adapter records.',
...
PROP_LIST = '\n'.join([
    c('list() reads a comma separated system property into a lower-cased list, falling back to the default shipped with the rule; the lists are tuned in the properties, not in the script.'),
...
def rule_420():
    h = header('USEM Management Interface Match',
        ...
        'hosts whose label ends with a listed controller suffix, or whose OS text names a controller (properties usem.ci_lookup.mgmt_suffixes and usem.ci_lookup.mgmt_os_markers).',
        'USEM Network Interface Name Match and USEM FQDN Name Hardware Match.')
    body = [OPEN, CHECK,
        "    var full = ('' + sourceValue).trim().toLowerCase();   // e.g. \"tx6dd630001-ilo.bankofamerica.com\"",
        ...
        stage('Recognise a controller and derive the server name',
              'The segment after the last hyphen is checked against the suffix list; when it is listed the server name is the label without it. Otherwise the OS text is checked for a controller word, and then the last segment is stripped whatever it is, because some controllers carry a site-specific tail instead ("crpchictx103-r" with OS "HP iLO"). Both habits are in the feed. Without either piece of evidence the host is not a controller and the rule must not touch it ("usposwks0042-x" with a Windows OS stays with the name rules).'),
...
def rule_430():
    h = header('USEM Network Interface Name Match',
        ...
        'the hostname rules tried the whole label and USEM Management Interface Match looked for a controller suffix.',
        'hosts whose DNS domain is an interface domain (".network." in our feed, property usem.ci_lookup.interface_domains) or whose label contains an interface marker segment such as "vlan705", "v201", "hsrp", "aom" (property usem.ci_lookup.interface_markers).',
...
def rule_460():
    ...
    h = header('USEM Load Balancer Service Match',
        ...
        'the hardware rules declined: a VIP has no serial, its name is not a server name, and the address belongs to a load balancer device.',
        'hosts with VIP evidence: an OS text naming a load balancer product (property usem.ci_lookup.vip_os_markers) or a DNS label with a VIP marker segment such as "-vip" or "vs1" (property usem.ci_lookup.vip_markers).',
