...
def rule_455():
    ...
    body = [OPEN, CHECK, ip_prep(value),
        ...
        stage('The pool behind the virtual server',
              'A pool is reached three ways for each twin, because the load balancer model can be loaded either through reference fields or through relationships: the pool field on the service, pools whose service field points at the service, and pools related to the service. Without a pool the rule declines and the service rule attaches the virtual server.',
