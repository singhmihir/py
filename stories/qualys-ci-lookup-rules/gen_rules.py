...
POLISHED = {'350', '410', '415', '455', '700', '705', '730', '740'}   # re-issued 15 Sep with the aligned layout; the rest keep their delivered text
...
def rule_455():
    ...
    h = header('USEM Load Balancer Member Match',
        ...
        'the sys_id of the one real server behind the virtual server; null when the host shows no VIP sign, no single service matches, the service has no pool or no members, or two or more servers sit behind it.',
        ...
        'USEM Load Balancer Service Match, which attaches the virtual server record itself when this rule declines.')
    body = [OPEN, CHECK, ip_prep(value),
        ...
        stage('VIP sign first',
              'The rule goes on only when the OS text contains a listed load balancer word or one of the label segments is a listed VIP marker, exactly as the service rule does. An ordinary server that shares an address with a virtual IP is never walked through a pool.',
              '"f5 big ip" contains "f5", so sign is true. "rbps-dev3-sve-vip.ecommnp.rpg" with OS "Linux 2.6" would qualify through the segment "vip"; "ah-1047132-001" with OS "Red Hat Enterprise Linux 9.8" has neither and the rule would decline.'),
        c('The load balancer products looked for in the OS text, and the label segments that mark a virtual IP. Keep both lists the same as in the service rule.'),
        ...
        stage('The one virtual server, by fqdn, then name, then address',
              'The same search as the service rule: the scanned DNS name in the fqdn field, then in the name field, then the label in the name field, then the scanned address in ip_address. Each step accepts exactly one service; a step that finds two ends the rule; a step that finds nothing hands over to the next.',
              'the first step, fqdn "crisp-tx.bankofamerica.com", finds the Load Balancer Service "crisp-tx" and no second row, so service holds its sys_id.'),
        ...
        '        return out;',
        '    }',
        stage('The pool behind the virtual server',
              'A pool is reached three ways, because the load balancer model can be loaded either through reference fields or through relationships: the pool field on the service, pools whose service field points at the service, and pools related to the service. Without a pool the rule declines and the service rule attaches the virtual server.',
              'the service "crisp-tx" carries the pool "crisp-tx-pool" in its pool field; poolIds holds that one sys_id.'),
        '    var pools = {};',
        ...
        stage('The pool members and their addresses',
              ...
              'the pool "crisp-tx-pool" has one member, "crisp-tx-pool_10.10.20.31_443", with ip_address "10.10.20.31"; members holds that one entry.'),
        '    var members = {};',
        ...
        '    if (!memberIds.length)',
        '        return null;',
        stage('The real servers, exactly one',
              'Each member address is looked for on device records, on network adapters and on IP Address records, and each member is followed through its relationships to hardware; load balancer devices and ignored classes are left out. The distinct servers found are counted. One server is the match; none, or two or more, and the rule declines, leaving the virtual server record to the service rule. A member address carried by two device records is two servers and declines as well.',
              'the Linux Server "usvacrispweb01" carries "10.10.20.31" in its ip_address field, so servers holds one entry and its sys_id is returned. A second member on "10.10.20.32" owned by another server would make the rule decline and the service rule attach "crisp-tx".'),
        '    var servers = {};',
        '    function keep(id) {',
