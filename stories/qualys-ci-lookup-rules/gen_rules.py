...
def rule_460():
    value = '171.203.142.40'
    payload = {"ID": "1202267244", "IP": value, "TRACKING_METHOD": "IP", "OS": "F5 Big IP", "DNS": "horizon-vip.bankofamerica.com"}
    h = header('USEM Load Balancer Service Match',
        'A virtual IP answered by a load balancer is not the balancer, and the hardware rules refuse the balancer device on purpose. When the member rule could not name the one machine behind the virtual server, the Load Balancer Service CI that models the VIP can stand for the host, but only when the CMDB does not show several servers behind it: a pool with several member addresses, or members resolving to several machines, means the finding belongs to one of several hosts and nothing here can say which, so the host is left unmatched. This rule finds the virtual server the way the member rule does, walks the same pool, and attaches the record when the pool is unknown, empty, or fronts one server. It runs on the IP field so that VIPs without a DNS name are covered too.',
        payload, 'IP', value, 'the OS and the DNS name',
        'the sys_id of the one Load Balancer Service CI found by fqdn, then by name, then by address (a retired copy of the record set aside), when its pool is unknown, empty, or fronts one server; null when the host shows no VIP evidence, no service matches, two live records or two different names carry the value, or the pool shows several servers.',
        'the Load Balancer Service CI "horizon-vip", whose fqdn is "horizon-vip.bankofamerica.com" and which carries no pool in the CMDB.',
        'the hardware rules declined: a VIP has no serial, "horizon-vip" is not a server name, and the address belongs to a load balancer device; USEM Load Balancer Member Match found no pool to walk.',
        'hosts with VIP evidence (an OS text naming a load balancer product, or a DNS label with a VIP marker segment such as "-vip" or "vs1"; both lists are declared in the script) whose real server the CMDB cannot name.',
        'the IP address rules, for hosts that are not VIPs.')
    body = [OPEN, CHECK, ip_prep(value),
        "    var dns = ('' + (sourcePayload.DNS || '')).trim().toLowerCase();   // \"horizon-vip.bankofamerica.com\", may be empty",
        "    var label = dns.split('.')[0];                // \"horizon-vip\"",
        "    var os = ('' + (sourcePayload.OS || '')).toLowerCase();   // \"f5 big ip\"",
        IGNORE_BLOCK, SEG_MATCH,
        lb_sign_stage('evidence', '"f5 big ip" contains "f5" and the label segment "vip" is a marker, so evidence is true. "rbps-dev3-sve-vip.ecommnp.rpg" with OS "Linux 2.6" would qualify through the segment alone; "ah-1047132-001" with OS "Red Hat Enterprise Linux 9.8" has neither and the rule would decline.'),
        lb_service_stage('the first step, fqdn "horizon-vip.bankofamerica.com", finds the Load Balancer Service "horizon-vip" and no second row, so twins holds that one record and service its sys_id. Two live records named "horizon-vip" on "171.203.142.40", one per balancer of the pair, would both be kept and the rule would decline just below; a retired copy beside a live one would be set aside.'),
        stage('One record left',
              'Two records still standing, both devices of a pair live, are an ambiguity nothing here can settle: the rule declines rather than pick one.',
              'twins holds the one record "horizon-vip", so the rule goes on.'),
        '    if (twins.length > 1)',
        '        return null;                              // two live records of one virtual server compete, never guess',
        LB_WALK,
        lb_pools_stage('The same walk as the member rule: the pool field on the service, pools whose service field points at the service, and pools related to the service. Without a pool the CMDB shows nothing behind the virtual server, and its record is the best CI there is: it is returned.',
                       'the service "horizon-vip" carries no pool, no pool points at it and none is related to it, so poolIds is empty and the sys_id of "horizon-vip" is returned.', 'return service;'),
        lb_members_stage('not reached for the sample. A pool "horizon-vip-pool" with the members "horizon-vip-pool_10.10.30.11_443" and "horizon-vip-pool_10.10.30.12_443" would put two entries in members. A pool without members returns the virtual server record, as an absent pool does.', 'return service;'),
        lb_servers_stage('Several servers behind the virtual server: never one CI',
              'The members are placed exactly as in the member rule, and the distinct member identities (addresses, or the record for a member without one) and the machines they lead to are counted. One machine that every member leads to is the case the member rule already handles; the virtual server record stands for it here only when that rule left it (two live records of the machine). Several identities or several machines mean the finding belongs to one of several hosts, and the rule declines: the host stays unmatched rather than carry a CI that is not its own. One identity that the CMDB cannot place is one server the CMDB does not hold, and the virtual server record stands for it.',
              'not reached for the sample. The two members "10.10.30.11" and "10.10.30.12" would be two identities, owned by "usvahorizon01" and "usvahorizon02" or by nobody, and the rule would decline; a single member "10.10.30.11" that no CI carries would return the record "horizon-vip".'),
        ...
        '        return null;                              // several servers behind the virtual server, never guess',
        '    return service;',
        CLOSE]
