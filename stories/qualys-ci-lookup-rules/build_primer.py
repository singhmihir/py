...
table(ws, 4, ['Question', 'Answer'], [
 ...
 ('Why does each rule have a "Class" version and a "Hardware" version?', 'Narrow first: inside the class the OS points at, where a namesake of another type cannot appear. Wide second: the whole hardware tree, for hosts whose OS is unknown (a third of the feed) or whose CI is stored as a plain Server. The wide rules, and the rules that follow discovery records, still refuse a CI whose class does not fit the OS: for a Cisco IOS host, Network Gear or Hardware is fine, a Computer is not.'),
