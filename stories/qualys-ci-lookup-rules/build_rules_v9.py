...
decline. V1.2 (same day): 460 attaches the virtual server record again without walking the pool; the pool walk belongs
to 455 alone."""
import os, sys, json
...
NAME = 'SNOWUSEMTP-895_MS_Qualys CI Lookup Rules Load Balancer Refinements_V1.2'
DESC = ('Three USEM Qualys CI lookup rules re-issued. 350 Layered DNS Match refuses a load balancer device at the end of the DNS Name -> IP '
        ...
        'decline, because the pool is then partly unknown, and the service rule attaches the virtual server record as before. No guess is '
        'made anywhere: an ambiguity the CMDB cannot settle is left to the service rule or unmatched. No system property is read or shipped.')
